...
const IncidentAdminPage = () => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    url: Yup.string().url(),
    abstract: Yup.string().required("incident abstract is required"),
  ...
  const deleteIncident = (incident) => {
    Swal.fire({
      ...
      html:
        ...
        "<br/>" +
        "Date:" +
        moment(incident.incident_time).format("MM/DD/YYYY") +
        "<br/>" +
        "Location:" +
  ...
  const stateAbbrOptions = (
    <>
      {statesAbbreviation.map((abbr) => (
        <option key={abbr[0]} value={abbr[0]}>
          {abbr[1]}
        </option>
      ))}
    </>
    ...
    <div className="mx-auto w-11/12 md:w-2/4 py-8 px-4 md:px-8">
      <div className="flex border flex-col items-center md:flex-row md:items-start border-blue-400 px-3 py-4">
        ...
        <p className="text-2xl font-semibold">
          Name: {displayName} <br /> Email: {email}
        </p>
        <p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} onReset={reset}>
        ...
        <div className="row">
          <div className="mb-3 col-2">
            ...
            <input
              ...
              className={`form-control ${
                errors?.incident_time ? "is-invalid" : ""
              }`}
          ...
          <div className="mb-3 col-2">
            ...
            <select
              ...
              className={`form-control ${
                errors?.incident_location ? "is-invalid" : ""
              }`}
            >
            ...
            <div className="invalid-feedback">
              {errors?.incident_location?.message}
            </div>
          </div>
        ...
        <div className="row">
          <div className="mb-3 col-8">
            ...
            <input
              {...register("donation_link")}
              type="text"
              className={`form-control ${
                errors?.donation_link ? "is-invalid" : ""
              }`}
            />
            <div className="invalid-feedback">
              {errors?.donation_link?.message}
            </div>
          </div>
          <div className="mb-3 col-4">
            <label>Police Tip Line</label>
            <input
              {...register("police_tip_line")}
              type="text"
              className={`form-control ${
                errors?.police_tip_line ? "is-invalid" : ""
              }`}
            />
            <div className="invalid-feedback">
              {errors?.police_tip_line?.message}
            </div>
          </div>
        </div>
        <div className="form-row">
          <div className="mb-3 col-12">
            ...
            <textarea
              {...register("help_the_victim")}
              rows="4"
              className={`form-control ${
                errors?.help_the_victim ? "is-invalid" : ""
              }`}
            />
            <div className="invalid-feedback">
              {errors?.help_the_victim?.message}
            </div>
          </div>
        </div>
        <div className="mb-3">
