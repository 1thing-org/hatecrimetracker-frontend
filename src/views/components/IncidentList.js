      ...
      <div>
        {incident?.donation_link ? (
          <div className="row">
            ...
            <div className="col">
              <a
                className="donation-link link-text"
                href={incident?.donation_link}
                target="_blank"
              >
                {t("make_a_donation")}
              </a>
        ...
        {incident?.police_tip_line ? (
          <div className="row">
            <img
              className="icon col"
              src={policeTipLineIcon}
              alt="police tip line"
            />
            <p className="description col">
              <b>{t("police_tip_line")}</b>: {incident?.police_tip_line}
            </p>
          </div>
        ) : null}
        {incident?.help_the_victim ? (
          <div className="row">
            <img
              className="icon col"
              src={helpTheVictimIcon}
              alt="help the victim"
            />
            <p className="description col">
              <b>{t("other_ways_to_help")}</b>: {incident?.help_the_victim}
            </p>
          </div>
        ) : null}
      </div>
    ...
    <div>
      ...
      <div className="incident-list">
        {props.data.map(function (d, idx) {
          ...
          if (
            searchTerm == "" ||
            getTitle(d).toLowerCase().includes(searchTerm.toLowerCase()) ||
            ...
            return (
              <div className="incident" key={idx}>
                ...
                <p className="location-time">
                  {stateFullName(d.incident_location)} |{" "}
      ...
      >
        ...
        <p className="location-time">
          {stateFullName(modalData?.incident_location)} |{" "}
          {moment(modalData?.incident_time).format("MM/DD/YYYY")}
        </p>
        <br></br>
        <p className="description">{getAbstract(modalData)}</p>
        <br></br>
        <a className="link-text" href={modalData?.url} target="_blank">
          {t("link_to_news_source")}
        </a>
      ...
      {visibleLimit < props.data.length && searchTerm == "" ? (
        <div align="center">
          <Button
            ...
            size="sm"
            onClick={() => setVisibleLimit(visibleLimit + INCR_COUNT)}
          >
            {t("load_more")}.
          </Button>
        </div>
