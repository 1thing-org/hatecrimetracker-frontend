...
import {
  stateFullName,
  getStateIncidentPer10kAsian,
  formatIncidentRate,
} from "../../utility/Utils";
import { useTranslation } from "react-i18next";

// monthly_data={'2021-01':100, '2021-02': 50...}
const IncidentChartPer10kAsian = ({
  color,
  monthly_stats,
  date_range,
  state,
  ...
  useEffect(() => {
    ...
    for (
      let x_day = startDate;
      x_day < lastDate;
      x_day = dateFns.addMonths(x_day, 1)
    ) {
      ...
      if (monthly_stats[month]) {
        ...
        monthly_data.push({
          ...
          "cases_per_10k": formatIncidentRate(
            getStateIncidentPer10kAsian(monthly_stats[month], state)
          ),
        });
      } else {
        monthly_data.push({
    ...
    if (active && payload && payload[0] && payload[0].value) {
      ...
      return (
        <div className="recharts-custom-tooltip">
          ...
          <p>
            <strong>
              {t("incident_chart.total_monthly_cases_per_10k_Asian", {
                count: monthly,
              })}
            </strong>
          </p>
          <p>
            <strong>
              {t("incident_map.count_10k_asian") + " : " + monthly_10k_asian}
            </strong>
          </p>
        </div>
      );
        ...
        <div className="recharts-wrapper">
          <ResponsiveContainer>
            <BarChart height={300} data={monthlyData}>
              ...
              <Bar
                name={t("monthly_per_10k_asian")}
                dataKey="cases_per_10k"
                stroke={monthlyData.length > 60 ? color : undefined}
                fill={color}
                strokeWidth={3}
              />
              <Legend
                wrapperStyle={{ position: "relative", marginTop: "4px" }}
              />
            </BarChart>
          </ResponsiveContainer>
        </div>
